...
object IntentHelper {
    ...
    private fun addUUIDToIntent(uuid:String?,campaign_name:String?,intent: Intent){
        if(!uuid.isNullOrEmpty() && !campaign_name.isNullOrEmpty()){
            intent.putExtra(SmartNotificationsConstants.KEY_NOTIFICATION_REPORT_UUID,uuid)
            intent.putExtra(SmartNotificationsConstants.KEY_NOTIFICATION_REPORT_CAMPAIGN_NAME,campaign_name)
        }
    }
